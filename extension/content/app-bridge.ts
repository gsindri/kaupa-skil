const log = (...args: any[]) => console.log(new Date().toISOString(), '[ab]', ...args);

;(window as any).__KpsExtPresent = true;
...
window.addEventListener('message', async e => {
  ...
  if (data.type === 'KPS_REQUEST_ORIGIN') {
    ...
    if (chrome.runtime.lastError) log('perm.request', chrome.runtime.lastError.message);
