    ...
    queryFn: async (): Promise<SupplierOption[]> => {
      ...
      const offers = await Promise.all(items.map(async (item: any) => {
        ...
        const cartItem = {
          ...
          packPrice: price?.pack_price ?? null,
          unitPriceExVat: price?.unit_price_ex_vat ?? null,
          unitPriceIncVat: price?.unit_price_inc_vat ?? null,
          quantity: 1,
          vatRate: 0.24,
          unit: 'unit',
        ...
        return {
          ...
          name: supplierName,
          packSize: `${item.pack_qty}`,
          packPriceExVat: price?.pack_price ?? null,
          packPriceIncVat: price?.pack_price ? price.pack_price * 1.24 : null,
          unitPriceExVat: price?.unit_price_ex_vat ?? null,
          unitPriceIncVat: price?.unit_price_inc_vat ?? null,
          stock: true,
