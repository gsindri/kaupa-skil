...
if (!fromItemId || !toItemId) {
  console.error('Usage: ts-node scripts/manual-merge.ts <fromItemId> <toItemId> [tenantId]')
  process.exit(1)
}
...
async function run() {
  ...
  await supabase.rpc('log_audit_event', {
    ...
    reason_text: 'manual merge',
    meta_data_val: { to_item_id: toItemId },
    tenant_id_val: tenantId ?? null,
  })

  console.log(`Merged item ${fromItemId} into ${toItemId}`)
}
