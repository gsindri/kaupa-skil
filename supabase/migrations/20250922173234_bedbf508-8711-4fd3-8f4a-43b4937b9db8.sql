-- Create organization catalog view as a function
...
AS $$
  SELECT 
    ...
    COALESCE(
      (ARRAY_AGG(derive_availability_status(sp.availability_text)) FILTER (WHERE sp.availability_text IS NOT NULL))[1], 
      'UNKNOWN'
    ) as availability_status,
    (ARRAY_AGG(sp.availability_text) FILTER (WHERE sp.availability_text IS NOT NULL))[1] as availability_text,
    (ARRAY_AGG(sp.updated_at) ORDER BY sp.updated_at DESC)[1] as availability_updated_at,
