-- Fix the view by dropping with CASCADE and recreating everything

-- Drop the existing view and dependent functions
DROP VIEW IF EXISTS v_public_catalog CASCADE;

-- Create availability status enum (if not exists)
DO $$ BEGIN
...
-- Recreate the v_public_catalog view with availability columns
CREATE VIEW v_public_catalog AS
WITH availability_data AS (
  SELECT 
    sp.catalog_product_id,
    sp.id as supplier_product_id,
    -- Derive availability from availability_text in supplier_product
    CASE 
...
-- Recreate the v_org_catalog function that was dropped
...
-- Also recreate the no-parameter version
