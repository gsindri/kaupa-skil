-- Fix v_public_catalog view to handle Icelandic availability text and add on_special column
...
SELECT 
  ...
  -- Map availability text to status (handle Icelandic text)
  CASE 
    WHEN (array_agg(sp.availability_text ORDER BY sp.updated_at DESC) FILTER (WHERE sp.availability_text IS NOT NULL AND sp.active_status = 'ACTIVE'))[1] ILIKE '%til á lager%' 
         OR (array_agg(sp.availability_text ORDER BY sp.updated_at DESC) FILTER (WHERE sp.availability_text IS NOT NULL AND sp.active_status = 'ACTIVE'))[1] ILIKE '%in stock%'
    ...
    WHEN (array_agg(sp.availability_text ORDER BY sp.updated_at DESC) FILTER (WHERE sp.availability_text IS NOT NULL AND sp.active_status = 'ACTIVE'))[1] ILIKE '%lítið magn%' 
         OR (array_agg(sp.availability_text ORDER BY sp.updated_at DESC) FILTER (WHERE sp.availability_text IS NOT NULL AND sp.active_status = 'ACTIVE'))[1] ILIKE '%low stock%'
    ...
    WHEN (array_agg(sp.availability_text ORDER BY sp.updated_at DESC) FILTER (WHERE sp.availability_text IS NOT NULL AND sp.active_status = 'ACTIVE'))[1] ILIKE '%ekki til á lager%' 
         OR (array_agg(sp.availability_text ORDER BY sp.updated_at DESC) FILTER (WHERE sp.availability_text IS NOT NULL AND sp.active_status = 'ACTIVE'))[1] ILIKE '%out of stock%'
  ...
  -- Add on_special column (placeholder for now)
  false as on_special,
