-- Phase 2: Live Pricing Infrastructure (Final)

-- Enable btree_gist extension for EXCLUDE constraint
CREATE EXTENSION IF NOT EXISTS btree_gist;

-- 1. Create supplier_offer table for time-bound pricing
CREATE TABLE IF NOT EXISTS public.supplier_offer (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  supplier_product_id UUID NOT NULL REFERENCES public.supplier_product(id) ON DELETE CASCADE,
...
-- Index for fast current offer lookup (without WHERE clause with NOW())
CREATE INDEX idx_supplier_offer_product_time 
ON public.supplier_offer (supplier_product_id, valid_from DESC, valid_to);

-- 2. Add offer_id to order_lines to track price source
ALTER TABLE public.order_lines
ADD COLUMN IF NOT EXISTS offer_id UUID REFERENCES public.supplier_offer(id);

-- 3. Add price validation timestamp to orders
ALTER TABLE public.orders
ADD COLUMN IF NOT EXISTS prices_last_validated_at TIMESTAMPTZ;

-- 4. Create function to get current offer for a product
...
-- 5. Create checkout price validation function (atomic transaction)
...
-- 6. RLS policies for supplier_offer
ALTER TABLE public.supplier_offer ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can view offers"
ON public.supplier_offer FOR SELECT
TO authenticated
USING (true);

CREATE POLICY "System can manage offers"
ON public.supplier_offer FOR ALL
TO authenticated
USING (true)
WITH CHECK (true);

COMMENT ON TABLE public.supplier_offer IS 'Time-bound pricing offers from suppliers for live pricing system';
COMMENT ON FUNCTION public.checkout_validate_prices IS 'Validates and updates cart prices atomically before checkout, detecting price drift';
COMMENT ON FUNCTION public.get_current_offer IS 'Returns the currently valid offer ID for a supplier product';